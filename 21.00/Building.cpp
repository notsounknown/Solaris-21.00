...
void Building::ServerCreateBuildingAndSpawnDeco(UObject* Context, FFrame& Stack) {
	...
	UEAllocatedWString MatName;
	UEAllocatedWString MatNameShort;
	switch ((int)Mat) {
	case 0:
		MatName = L"Wood";
		MatNameShort = L"W";
	...
	UEAllocatedWString BuildingType;
	switch ((int)InBuildingAttachmentType) {
	case 0:
	case 6:
	case 7:
	...
	auto Build = Utils::FindObject<UClass>(L"/Game/Building/ActorBlueprints/Player/" + MatName + L"/L1/PBWA_" + MatNameShort + L"1_" + BuildingType + L".PBWA_" + MatNameShort + L"1_" + BuildingType + L"_C");

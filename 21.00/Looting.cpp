...
{
	FName DefaultLootTierGroup = ((ABuildingContainer*)BuildingContainer->Class->DefaultObject)->SearchLootTierGroup;

	if (BuildingContainer->SearchLootTierGroup != DefaultLootTierGroup)
	{
	...
	Log(L"ABuildingContainer::PostUpdate Setting ReplicatedLootTier to %i for %s with default %s",
		BuildingContainer->ReplicatedLootTier, BuildingContainer->SearchLootTierGroup.ToWString().c_str(), DefaultLootTierGroup.ToWString().c_str());
...
void Looting::SpawnConsumableActor(ABGAConsumableSpawner* Spawner) {
	...
	float AmmoSpawnMin = Utils::EvaluateScalableFloat(ItemDefinition->MinNumberConsumablesToSpawn), AmmoSpawnMax = Utils::EvaluateScalableFloat(ItemDefinition->MaxNumberConsumablesToSpawn);
	auto NumToSpawn = AmmoSpawnMin + (rand() % (int)(AmmoSpawnMax - AmmoSpawnMin));
	auto GroundLoc = UFortKismetLibrary::FindGroundLocationAt(UWorld::GetWorld(), nullptr, Spawner->K2_GetActorLocation(), -1000, 2500, L"FindGroundLocationAt"); // move to ground

...
void Player::ServerAttemptAircraftJump(UObject* Context, FFrame& Stack)
{
	...
	if (PlayerController->MyFortPawn) {
		...
		if (bLateGame) {
			...
			Entries[0] = Inventory::GiveItem(PlayerController, AssaultRifle.Item, AssaultRifle.Count, AssaultRifleClipSize, true);
			Entries[1] = Inventory::GiveItem(PlayerController, Shotgun.Item, Shotgun.Count, ShotgunClipSize, true);
			Entries[2] = Inventory::GiveItem(PlayerController, Sniper.Item, Sniper.Count, SniperClipSize, true);
			Entries[4] = Inventory::GiveItem(PlayerController, Heal.Item, Heal.Count, HealClipSize, true);
			Entries[3] = Inventory::GiveItem(PlayerController, HealSlot2.Item, HealSlot2.Count, HealSlot2ClipSize, true);
		}
	}
}
...
void Player::ServerAttemptInteract(UObject *Context, FFrame& Stack)
{
	...
	auto InteractionComp = (UFortControllerComponent_Interaction*)Context;
	...
	return callOG(InteractionComp, Stack.CurrentNativeFunction, ServerAttemptInteract, ReceivingActor, InteractComponent, InteractType, OptionalObjectData, InteractionBeingAttempted, RequestId);
}
...
void Player::Hook()
{
	...
	Utils::ExecHook(L"/Script/FortniteGame.FortControllerComponent_Interaction.ServerAttemptInteract", ServerAttemptInteract, ServerAttemptInteractOG);
